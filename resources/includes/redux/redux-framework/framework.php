...
if ( ! class_exists( 'ReduxFramework' ) ) {
	...
	class ReduxFramework {
		...
		public static function init() {
			...
			self::$wp_content_url = trailingslashit( Redux_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://',
				'https://', WP_CONTENT_URL ) : WP_CONTENT_URL ) ) );

			// See if Redux is a plugin or not
			if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ),
					Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ),
					Redux_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ),
					Redux_Helpers::cleanFilePath( WP_CONTENT_DIR . '/themes/' ) ) !== false
			) {
			...
			if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
				...
				if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ),
						Redux_Helpers::cleanFilePath( get_template_directory() ) ) !== false
				) {
					$relative_url = str_replace( Redux_Helpers::cleanFilePath( get_template_directory() ), '',
						self::$_dir );
					self::$_url   = trailingslashit( get_template_directory_uri() . $relative_url );
				} else if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ),
						Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false
				) {
					$relative_url = str_replace( Redux_Helpers::cleanFilePath( get_stylesheet_directory() ), '',
						self::$_dir );
		...
		public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
			...
			if ( empty ( $this->args['footer_credit'] ) ) {
				$this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s',
						'redux-framework' ),
						'<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework',
							'redux-framework' ) . '</a> v' . self::$_version ) . '</span>';
			}
			...
			if ( ! empty ( $this->args['opt_name'] ) ) {
				 ...
				 * @param  array $sections field option sections
				 */
				$this->sections = apply_filters( "redux-sections-{$this->args['opt_name']}",
					$this->sections ); // REMOVE LATER
				/**
				...
				if ( empty ( $this->args['output_location'] ) || in_array( 'frontend',
						$this->args['output_location'] )
				) {
...
// _tracking()
		...
		public function _get_default( $opt_name, $default = null ) {
			if ( $this->args['default_show'] == true ) {
				...
				$default = array_key_exists( $opt_name,
					$this->options_defaults ) ? $this->options_defaults[ $opt_name ] : $default;
			}
...
// _get_default()
		...
		public function get( $opt_name, $default = null ) {
			return ( ! empty ( $this->options[ $opt_name ] ) ) ? $this->options[ $opt_name ] : $this->_get_default( $opt_name,
				$default );
		}
// get()
...
// set()
		...
		private function set_global_variable() {
			if ( $this->args['global_variable'] ) {
				 ...
				 */
				$GLOBALS[ $this->args['global_variable'] ] = apply_filters( "redux/options/{$this->args['opt_name']}/global_variable",
					$this->options );
...
//return $data;
			 ...
			 * @param string $data
			 */
			$data = apply_filters( "redux/options/{$this->args['opt_name']}/wordpress_data/$type/",
				$data ); // REMOVE LATER
...
// show()
		...
		public function _default_values() {
			...
			$this->options_defaults             = apply_filters( "redux/options/{$this->args['opt_name']}/defaults",
				$this->options_defaults, $this->transients['changed_values'] );
		...
		private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
			...
			if ( isset ( $test[ $page_parent ] ) ) {
				...
				if ( true == $addMenu ) {
					...
					$this->page = call_user_func( 'add_submenu_page', $page_parent, $page_title, $menu_title,
						$page_permissions, $page_slug, array(
							&$this,
							'generate_panel'
						) );
				}
		...
		public function _options_page() {
			...
			if ( $this->args['menu_type'] == 'hidden' ) {
				...
				$this->add_submenu(
					$this->args['page_parent'], $this->args['page_title'], $this->args['menu_title'],
					$this->args['page_permissions'], $this->args['page_slug']
				);
			} else {
				// Theme-Check notice is displayed for WP.org theme devs, informing them to NOT use this.
				$this->page = call_user_func( 'add_menu_page', $this->args['page_title'], $this->args['menu_title'],
					$this->args['page_permissions'], $this->args['page_slug'], array(
						&$this,
						'generate_panel'
				...
				if ( true === $this->args['allow_sub_menu'] ) {
					if ( ! isset ( $section['type'] ) || $section['type'] != 'divide' ) {
						foreach ( $this->sections as $k => $section ) {
							...
							// ONLY for non-wp.org themes OR plugins. Theme-Check alert shown if used and IS theme.
							call_user_func( 'add_submenu_page', $this->args['page_slug'], $section['title'],
								$section['title'], $this->args['page_permissions'],
...
// _admin_bar_menu()
		...
		public function _enqueue_output() {
					...
					foreach ( $section['fields'] as $fieldk => $field ) {
						if ( isset ( $field['type'] ) && $field['type'] != "callback" ) {
							...
							if ( ! class_exists( $field_class ) ) {
								 ...
								 */
								$class_file = apply_filters( "redux/{$this->args['opt_name']}/field/class/{$field['type']}",
									self::$_dir . "inc/fields/{$field['type']}/field_{$field['type']}.php", $field );
							...
							if ( ! empty ( $this->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class,
									'output' ) && $this->_can_output_css( $field )
							) {
			...
			if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'],
				...
				if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
					...
					wp_register_style( 'redux-google-fonts-' . $this->args['opt_name'],
						$protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
...
// _enqueue()
		...
		public function _load_page() {
			...
			if ( true == $this->show_hints ) {
				...
				$hint_status = get_user_meta( $current_user->ID, 'ignore_hints' ) ? get_user_meta( $current_user->ID,
					'ignore_hints', true ) : 'true';
				...
				// Construct message
				$msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.',
						'redux-framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;
...
// get_default_output_string()
		...
		public function get_header_html( $field ) {
			...
			if ( $this->args['default_show'] == true && isset ( $field['default'] ) && isset ( $this->options[ $field['id'] ] ) && $this->options[ $field['id'] ] != $field['default'] && ! in_array( $field['type'],
					$filter_arr )
			) {
		...
		public function _register_settings() {
			...
			foreach ( $this->sections as $k => $section ) {
				 ...
				 */
				if ( isset ( $section['id'] ) ) {
					$section = apply_filters( "redux/options/{$this->args['opt_name']}/section/{$section['id']}",
						$section );
				}
				...
				if ( isset ( $section['fields'] ) ) {
					foreach ( $section['fields'] as $fieldk => $field ) {
						 ...
						 */
						$field = apply_filters( "redux/options/{$this->args['opt_name']}/field/{$field['id']}/register",
							$field );
						...
						if ( $field['type'] == "media" ) {
							if ( isset ( $this->options[ $field['id'] ]['id'] ) && isset ( $this->options[ $field['id'] ]['url'] ) && ! empty ( $this->options[ $field['id'] ]['url'] ) && strpos( $this->options[ $field['id'] ]['url'],
									str_replace( 'http://', '', WP_CONTENT_URL ) ) === false
							) {
						...
						if ( $field['type'] == "background" ) {
							if ( isset ( $this->options[ $field['id'] ]['media']['id'] ) && isset ( $this->options[ $field['id'] ]['background-image'] ) && ! empty ( $this->options[ $field['id'] ]['background-image'] ) && strpos( $this->options[ $field['id'] ]['background-image'],
									str_replace( 'http://', '', WP_CONTENT_URL ) ) === false
							) {
						...
						if ( $field['type'] == "slides" ) {
							if ( isset ( $this->options[ $field['id'] ] ) && is_array( $this->options[ $field['id'] ] ) && isset ( $this->options[ $field['id'] ][0]['attachment_id'] ) && isset ( $this->options[ $field['id'] ][0]['image'] ) && ! empty ( $this->options[ $field['id'] ][0]['image'] ) && strpos( $this->options[ $field['id'] ][0]['image'],
									str_replace( 'http://', '', WP_CONTENT_URL ) ) === false
							) {
						 ...
						 * @param array $field field config
						 */
						$field = apply_filters( "redux-field-{$field['id']}modifier-{$this->args['opt_name']}",
							$field ); // REMOVE LATER
						 ...
						 * @param array $field field config
						 */
						$field = apply_filters( "redux/options/{$this->args['opt_name']}/field/{$field['id']}",
							$field );
						...
						if ( isset ( $this->args['display_source'] ) ) {
							$th .= '<div id="' . $field['id'] . '-settings" style="display:none;"><pre>' . var_export( $this->sections[ $k ]['fields'][ $fieldk ],
									true ) . '</pre></div>';
						...
						if ( ! $display || isset ( $this->no_panel_section[ $k ] ) ) {
							...
							if ( $this->args['options_api'] == true ) {
								add_settings_field(
									...
									"{$this->args['opt_name']}{$k}_section", $field
								);
							}
						}
					}
			...
			if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
				 ...
				 */
				do_action( "redux-compiler-{$this->args['opt_name']}", $this->options, $this->compilerCSS,
					$this->transients['changed_values'] ); // REMOVE
...
// _register_settings()
		...
		private function _register_extensions() {
			...
			foreach ( $folders as $folder ) {
				if ( $folder === '.' || $folder === '..' || ! is_dir( $path . $folder ) || substr( $folder, 0,
						1 ) === '.' || substr( $folder, 0, 1 ) === '@' || substr( $folder, 0, 4 ) === '_vti'
				) {
					continue;
				}
				 ...
				 */
				$class_file = apply_filters( "redux-extensionclass-load", "$path/$folder/extension_{$folder}.php",
					$extension_class ); // REMOVE LATER
				 ...
				 * @param string $extension_class          extension class name
				 */
				$class_file = apply_filters( "redux/extension/{$this->args['opt_name']}/$folder",
					"$path/$folder/extension_{$folder}.php", $class_file );
...
//print_r($plugin_options);
			...
			if ( ! empty ( $plugin_options['defaults'] ) ) {
				 ...
				 */
				$plugin_options = apply_filters( "redux/validate/{$this->args['opt_name']}/defaults",
			...
			if ( ! empty ( $plugin_options['defaults-section'] ) ) {
				if ( isset ( $plugin_options['redux-section'] ) && isset ( $this->sections[ $plugin_options['redux-section'] ]['fields'] ) ) {
					...
					$plugin_options = apply_filters( "redux/validate/{$this->args['opt_name']}/defaults_section",
						$plugin_options );
				}
...
//                }
			 ...
			 * @param  &array [&$plugin_options, redux_options]
			 */
			$plugin_options = apply_filters( "redux/validate/{$this->args['opt_name']}/before_validation",
		...
		public function ajax_save() {
			if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
				echo json_encode( array(
					'status' => __( 'Invalid security credential.  Please reload the page and try again.',
						'redux-framework' ),
					'action' => ''
				) );
			...
			if ( ! current_user_can( $this->args['page_permissions'] ) ) {
				echo json_encode( array(
					'status' => __( 'Invalid user capability.  Please reload the page and try again.',
						'redux-framework' ),
					'action' => ''
				) );
			...
			if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
				...
				if ( ! empty ( $values ) ) {
					...
					echo json_encode( array(
						'status' => __( 'Your panel has no fields. Nothing to save.', 'redux-framework' )
					) );
				}
			}
			if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
			...
			if ( isset( $return_array ) ) {
				...
				echo json_encode( apply_filters( "redux/options/{$this->args['opt_name']}/ajax_save/response",
					$return_array ) );
			}
				...
				if ( isset ( $section['fields'] ) ) {
					foreach ( $section['fields'] as $fkey => $field ) {
						...
						if ( ! isset ( $field['id'] ) || ! isset ( $plugin_options[ $field['id'] ] ) || ( isset ( $plugin_options[ $field['id'] ] ) && $plugin_options[ $field['id'] ] == '' ) ) {
							...
							// FIXME Even if a validate_callback is set this bails out in latest version
							// original check: if ( ! $isNotEmpty )
							if ( ! $isNotEmpty && ! isset ( $field['validate_callback'] ) ) {
						...
						if ( isset ( $field['validate'] ) ) {
							...
							if ( ! class_exists( $validate ) ) {
								...
								$class_file = apply_filters( "redux-validateclass-load",
									self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php",
									$validate ); // REMOVE LATER
								 ...
								 */
								$class_file = apply_filters( "redux/validate/{$this->args['opt_name']}/class/{$field['validate']}",
									self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php",
									$class_file );
							...
							if ( class_exists( $validate ) ) {
								...
								if ( isset ( $plugin_options[ $field['id'] ] ) && is_array( $plugin_options[ $field['id'] ] ) && ! empty ( $plugin_options[ $field['id'] ] ) ) {
									...
									$validation                     = new $validate ( $this, $field, $pofi,
										$options[ $field['id'] ] );
		...
		public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
			...
			$string = "";
			if ( ( ( isset ( $this->args['icon_type'] ) && $this->args['icon_type'] == 'image' ) || ( isset ( $section['icon_type'] ) && $section['icon_type'] == 'image' ) ) || ( isset( $section['icon'] ) && strpos( $section['icon'],
						'/' ) !== false )
			) {
...
//                    $class_file = apply_filters( 'redux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
				...
				if ( class_exists( $field_class ) ) {
					 ...
					 * @param array $field        field data
					 */
					$_render = apply_filters( "redux-field-{$this->args['opt_name']}", ob_get_contents(),
						$field ); // REMOVE
					 ...
					 * @param array $field        field data
					 */
					$_render = apply_filters( "redux/field/{$this->args['opt_name']}/{$field['type']}/render/after",
						$_render, $field );
...
//                        }
		...
		private function checkRequiredDependencies( $field, $data ) {
			...
			if ( ! in_array( $data['parent'],
					$this->fieldsHidden ) && ( ! isset ( $this->folds[ $field['id'] ] ) || $this->folds[ $field['id'] ] != "hide" )
			) {
				if ( isset ( $this->options[ $data['parent'] ] ) ) {
					$return = $this->compareValueDependencies( $this->options[ $data['parent'] ], $data['checkValue'],
						$data['operation'] );
